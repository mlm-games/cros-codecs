...
pub(crate) fn tunings_to_libva_rc<const CLAMP_MIN_QP: u32, const CLAMP_MAX_QP: u32>(
    ...
    let bits_per_second = tunings.rate_control.bitrate_maximum().unwrap_or(1_000_000);
    ...
    let target_percentage: u32 =
        if bits_per_second > 0 { 100 * target_bits_per_second / bits_per_second } else { 0 };
